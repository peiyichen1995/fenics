...
def NeoHookean_imcompressible(mu1, mu2, mu3, mu4, delta, beta3, beta4, a1, a2, F, Q):
    J = det(F)
    C = F.T * F
    temp1 = mu1 * pow(project(sqrt(tr(C)), Q), 2) / pow(J, 2 / 3)
    temp2 = mu2 * pow(project(sqrt(tr(matrix_cofactor(F).T *
                                      matrix_cofactor(F))), Q), 3) / pow(J, 2)
    h = pow(delta, beta3) + pow(delta, -beta3)
    temp3 = mu3 * h * J
    temp4 = mu4 / beta4 * \
        exp(beta4 * conditional(gt(inner(F * a1, F * a1), 1),
                                inner(F * a1, F * a1) - 1, 0))
    temp5 = mu4 / beta4 * \
        exp(beta4 * conditional(gt(inner(F * a2, F * a2), 1),
                                inner(F * a2, F * a2) - 1, 0))
...
Pi = NeoHookean_imcompressible(
    mu1, mu2, mu3, mu4, delta, beta3, beta4, a1, a2, F, Q) * dx - dot(-P * n, u) * ds(1)
